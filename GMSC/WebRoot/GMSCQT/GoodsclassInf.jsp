...
<html>
  <head>
	...
	<script type="text/javascript">
    	function page(name,n){
    		location.href="../webpage/good?action=checkClass&class_name="+name+"&page="+n;
    	};
    </script>
  </head>
    	...
    	<div class="body3">
    		<table class="body_t1">
				<tr>
					...
					<a href="../webpage/good?action=checkClass&class_name=${class_name}&page=${page.pagenow-1 }">上一页</a>
					<select onchange="page(${class_name },this.value)">
						<c:forEach begin="1" end="${page.pagecount }" var="n">
							<option value="${n }" <c:if test="${page.pagenow==n }">selected="selected"</c:if>>${n }</option>
						</c:forEach>
					</select>
					<a href="../webpage/good?action=checkClass&class_name=${class_name}&page=${page.pagenow+1 }">下一页</a>
					</td>
				</tr>
    		</table>
    	</div>
